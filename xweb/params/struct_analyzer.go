...
// StructCache is holding all the necessary information to decode a struct
...
func (s StructAnalyzer) cache(cache *StructCache, paths TagsPath, t reflect.Type) (containTags bool) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if IsImplementingUnmarshaler(metadata.Type) {
		...
		if !(hasValueTag || (fieldHasTag && (metadata.ImplementUnmarshaller || IsUnmarshallable(metadata.Type)))) {
			continue
		}
...
func (s StructAnalyzer) lookupTags(structField reflect.StructField, currentPaths TagsPath) (containTags bool) {
	...
	for _, tag := range s.tags {
		if _, alreadyDefined := alreadySet[tag]; alreadyDefined {
			continue
		}
